...
def load_clip(tag="ViT-L-14"):
    model, _, transform = load_clip_model(tag)
    return model, transform


def extract_features(model, transform, device, split, cfg, cache_subdir):
    cache_key = f"v2_l14_{split}_{cfg['dir']}_{cfg.get('subdir', 'none')}.pkl"
...
def main():
    ...
    for ds_name, cfg in DATASET_CFG.items():
        ...
        for sn in strategies:
            ...
            ds_results[sn] = {"mean": mean, "std": std, "per_seed": accs}
    ...
    for ds_name, dsr in all_results.items():
        ...
        c = dsr["anchor_capped"]
        n = dsr["anchor_weight"]
        print(f"  {ds_name:22s}  {u['mean']:>6.2f}±{u['std']:.2f}"
              f"  {c['mean']:>6.2f}±{c['std']:.2f}"
              f"  {n['mean']:>6.2f}±{n['std']:.2f}")

    out_path = out_dir / "active_learning_capped_l14.json"
